...
""" Finetuning the library models for question-answering on Machine Reading Comprehension (DistilBERT, Bert, XLM, XLNet)."""
...
from ee_mrc_model import BertForEventExtractionModel
...
from processor import SquadProcessor, MRCJsonlProcessor
from example_freature import MRCResult
from mrc_metrics import compute_predictions_logits, squad_evaluate
from utils_xlm import compute_predictions_log_probs
from utils_mrc import mrc_convert_examples_to_features
...
PROCESSORS = {
    ...
    "event": SquadProcessor,
    "event_role": SquadProcessor,
    "combined": MRCJsonlProcessor,
}
...
MODEL_CLASSES = {
    "bert": (BertConfig, BertForEventExtractionModel, BertTokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    model.zero_grad()
    train_iterator = trange(epochs_trained, int(args.num_train_epochs), desc="Epoch", disable=args.silent)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating", disable=args.silent):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            result = MRCResult(unique_id, to_list(start_logits[i]), to_list(end_logits[i]),logits=to_list(logits[i]))
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if args.dataset_name in PROCESSORS:
            processor = PROCESSORS[args.dataset_name]()
        else:
            processor = MRCJsonlProcessor()
        if evaluate:
            examples = processor.get_examples(args.data_dir, filename=args.predict_file, mode='dev')
        else:
            examples = processor.get_examples(args.data_dir, filename=args.train_file, mode='train')

        # examples = examples[:1000]

        features, dataset = mrc_convert_examples_to_features(
            examples=examples,
            tokenizer=tokenizer,
    ...
    if evaluate:
        outfile = open(os.path.join(args.output_dir, "dev_cached_file.jsonl"), "w")
    else:
        outfile = open(os.path.join(args.output_dir, "train_cached_file.jsonl"), "w")
    for feature in features:
        outfile.write(feature.toJSON()+"\n")
    outfile.close()

    if evaluate:
        outfile = open(os.path.join(args.output_dir, "dev_cached_examples.jsonl"), "w")
    else:
        outfile = open(os.path.join(args.output_dir, "train_cached_examples.jsonl"), "w")
...
def argument_parser():
    ...
    parser.add_argument("--dataset_name", default="squad", type=str, help="The name of the dataset.", )
    ...
    parser.add_argument("--silent", action="store_true", help="Silent when progress")
    ...
    parser.add_argument("--n_best_size", default=5, type=int, help="The total number of n-best predictions to generate in the nbest_predictions.json output file.", )
    ...
    parser.add_argument("--chinese", action="store_true", help="The test is in chinese")
    parser.add_argument("--num_event", type=int, default=2, help="random seed for initialization")
    ...
    parser.add_argument("--attention_size", type=int, default=768, help="the hidden size of attention")
    parser.add_argument("--loss_weight", type=int, default=1, help="the weight of loss that divided")
...
def main():
    ...
    config.num_event = args.num_event
    config.attention = args.attention
    config.loss_weight = args.loss_weight
    config.attention_size = args.attention_size
